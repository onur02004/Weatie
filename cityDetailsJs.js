...
function formatCityNameForUrl(cityName) {
  // Remove spaces and periods, assuming filenames like 'Weingarten.JPG', 'NewYork.JPG', 'StPetersburg.JPG'
...
async function getWeatherInfo() {
    ...
    try {
        ...
        document.querySelector('.mostImportantContainer .stat:nth-child(1) .number').textContent = Math.round(current.feelslike_c);
        ...
        document.querySelector('.mostImportantContainer .stat:nth-child(2) .number').textContent = 'N/A'; // Air Quality (Requires aqi=yes in URL)
        document.querySelector('.mostImportantContainer .stat:nth-child(3) .number').textContent = Math.round(current.wind_kph);
        ...
        const hourlyWeatherContainer = document.querySelector('.MainContainer > .dailyWeatherContainer:last-of-type');
